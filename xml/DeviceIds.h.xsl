...
<xsl:apply-templates select="//device" mode="cxx-style"/>
        ...
        const char* getName(ndlcom::deviceId::id id)
        {
            return ndlcomDeviceGetName((ndlcomDeviceId)id);
        }
        ndlcom::deviceId::id getId(const char* name)
        {
            return (ndlcom::deviceId::id)ndlcomDeviceGetId(name);
        }
    };
};
#endif/*__cplusplus*/
